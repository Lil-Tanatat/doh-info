...
import formConfig from "@/form/edit-user.json";
...
export function EditUserForm({
  ...
  const validateField = (field: FormField, value: string | string[]) => {
    ...
    // Additional field-specific validation (only for non-disabled fields)
    if (!field.disabled && typeof value === "string" && value.trim() !== "") {
      switch (field.name) {
        case "password":
    ...
    <form className={cn("space-y-8", className)} onSubmit={handleSubmit}>
      <Card className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-center pt-6 mb-10">
          ...
          >
            {isSubmitting ? "กำลังแก้ไขผู้ใช้งาน..." : "แก้ไขผู้ใช้งาน"}
          </Button>
        </div>
      </Card>
    </form>
